...
use crate::components::{Animation, Action, ActionId};
...
impl<'s> System<'s> for AnimationSystem {
  ...
  fn run(&mut self, (animations, actions, mut sprite_renders, time): Self::SystemData) {
    // Get every animation with its associated SpriteRender component.
    for (animation, action, sprite) in (&animations, &actions, &mut sprite_renders).join() {
